...
- [ ] T001 Create monorepo directories `backend/`, `frontend/`, `bot/`, `docs/`, `scripts/` per `specs/001-exchange-platform/plan.md`
- [ ] T002 Initialize `backend/package.json` with NestJS, CQRS, Prisma, BullMQ, Pino, Jest and TypeScript 5 / Node 20
- [ ] T003 [P] Initialize `frontend/package.json` with Vite, React, TanStack Query, Tailwind, Vitest
- [ ] T004 [P] Initialize `bot/package.json` with grammY or Telegraf TypeScript client
- [ ] T005 [P] Add root `Makefile`, `docker-compose.yml` (Postgres + Redis), `VERSION`, and `.gitignore`
- [ ] T006 [P] Add `backend/.env.example`, `frontend/.env.example`, `bot/.env.example` (worker secrets documented as worker-only)
- [ ] T007 [P] Configure ESLint/Prettier for `backend/`, `frontend/`, `bot/`
- [ ] T008 Create `AGENTS.md` stub pointing at constitution + `docs/DEVELOPMENT-DIRECTION.md`
...
- [ ] T010 [P] Add failing unit tests for order state machine allowlist in `backend/src/orders/domain/order-state-machine.spec.ts`
...
- [ ] T013 Implement order state machine in `backend/src/orders/domain/order-state-machine.ts` (make T010 pass)
- [ ] T014 Create Prisma schema skeleton (Customer, Asset refs, ExchangePair, PaymentMethod, Quote, Order, Payment, Payout, LedgerEntry, AuditEvent, ExceptionCase, OperatorUser, PlatformSettings) in `backend/prisma/schema.prisma`
...
- [ ] T021 Implement provider ports (`RateProvider`, `ExchangeProvider`, `PaymentProvider`, `PayoutProvider`) in `backend/src/exchange/domain/` and `backend/src/payments/domain/`
- [ ] T022 [P] Implement Mock providers behind `MOCK_PROVIDERS` in `backend/src/exchange/infrastructure/mock/` and `backend/src/payments/infrastructure/mock/`
...
- [ ] T028 Seed script: pairs USDT↔UAH + USDT↔BTC, UAH methods, operator user in `backend/prisma/seed.ts`
...
**Checkpoint**: `make dev-infra && make db-migrate && make db-seed && make dev-api && make dev-worker` boot; Money + state machine + ledger tests green

---

## Phase 3: User Story 1 — Fiat↔crypto USDT↔UAH (Priority: P1) 🎯 MVP

**Goal**: Customer completes USDT↔UAH both directions on web (quote → pay → payout)

**Independent Test**: Quickstart Scenario A + B with `MOCK_PROVIDERS=true`
...
- [ ] T033 [P] [US1] Failing e2e happy path USDT→UAH in `backend/test/exchange-uah-usdt.e2e-spec.ts`
...
- [ ] T037 [US1] Implement confirm quote → CreateOrder + payin instructions in `backend/src/orders/`
- [ ] T038 [US1] Implement PaymentMethod config + UAH card/IBAN adapters (mock) in `backend/src/payments/`
- [ ] T039 [US1] Implement `payment.detect` / `payment.confirm` processors with ledger+audit in `backend/src/worker/processors/payment-*.processor.ts` (make T034 pass)
- [ ] T040 [US1] Implement exchange routing + `payout.execute` for UAH out / USDT out in `backend/src/payouts/` and worker processors
...
- [ ] T043 [US1] Notifications stub (in-app/log) on status changes in `backend/src/notifications/`
- [ ] T044 [US1] Build web exchange flow UI in `frontend/src/features/exchange/` (amount → quote → confirm → status)
...
**Checkpoint**: USDT↔UAH happy path works on web without operator; MVP demoable

---

## Phase 4: User Story 2 — Crypto↔crypto USDT↔BTC (Priority: P1)

**Goal**: USDT↔BTC both directions using same engine + address/network validation
...
- [ ] T048 [P] [US2] Failing e2e USDT→BTC in `backend/test/exchange-usdt-btc.e2e-spec.ts`
...
**Checkpoint**: USDT↔BTC works independently with same lifecycle as US1

---

## Phase 5: User Story 3 — Track status & explain help (Priority: P2)

**Goal**: Clear statuses on web; explain/copy never invents payment confirmation; Telegram parity for list/detail
...
- [ ] T057 [US3] Implement `POST /api/v1/explain` sanitized context builder in `backend/src/explain/` (make T055 pass)
- [ ] T058 [US3] Add explain/help UI affordances in `frontend/src/features/orders/` and `frontend/src/features/exchange/`
- [ ] T059 [US3] Implement Telegram auth link + quote/order/list commands calling API in `bot/src/`
...
- [ ] T080 Run full `specs/001-exchange-platform/quickstart.md` scenarios A–G and record results in `specs/001-exchange-platform/checklists/quickstart-results.md`
...
|-------|------------|--------|
| US1 | Phase 2 | MVP; no other stories |
| US2 | Phase 2 + largely US1 engine | Can start after US1 quote/order/payment pipeline exists |
| US3 | Phase 2 + order read APIs (US1) | Telegram/explain after orders exist |
...
- T009–T011 together; T017–T019, T025–T026, T029–T030 after core prisma  
...
**T001–T046** (Setup + Foundational + US1). Defer US2+ until US1 e2e is green.
