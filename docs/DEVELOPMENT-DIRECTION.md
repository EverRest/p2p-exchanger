...
**Design reference:** [design v0.3](./superpowers/specs/2026-08-21-p2p-exchanger-design.md) · **Security:** [SECURITY.md](./SECURITY.md)
...
| Provider abstraction | `AiProvider` + registry + fallback | `ExchangeProvider`, `PaymentProvider`, `PayoutProvider`, `RateProvider`, `KycProvider` + failover (Binance → hot wallet) |
...
| Docs-first | `docs/domain`, `workflows`, `adr`, `AGENTS.md` | Same tree under exchanger `docs/` — **`docs/domain/` and `docs/workflows/` exist** (handbook H2) |
...
|------------|-----------|
| `TranslationJob` saga | `Order` state machine + payment/payout saga (Assisted: operator confirm/approve) |
...
| Approval workflow (human review copy) | Exception/dispute queue + mandatory `confirm_payment` / `approve_payout` (RBAC) |
| Copilot / AI in product | **Explain/copy in MVP** (no privileged tools) — isolate like AI calls; allowlist only; see [SECURITY.md](./SECURITY.md) |

## Frontend choice vs transl8.ai — decided

**React + Vite + feature folders + TanStack Query + Zustand + Tailwind + i18n (uk/en)** (same as transl8.ai).

Rationale: maximize reuse of agent playbooks, folder layout, and API-layer patterns; keep all money logic in Nest API/worker. Marketing/landing can be a separate static site later if needed — not the app shell.

## Bot & runtime — decided

| Choice | Decision |
|--------|----------|
| Telegram bot | **grammY** thin client → API (not Telegraf) |
| Node | **22** |
...
├── bot/                     # grammY thin client → API
├── docs/
│   ├── SECURITY.md
│   ├── system-overview.md
│   ├── architecture.md
│   ├── patterns.md
│   ├── domain/              # handbook entities (exists)
│   ├── workflows/           # Assisted lifecycle (exists)
│   ├── adr/
│   └── api/
└── scripts/
```
